...
function AdminPayments() {
  ...
  // Load payments from localStorage (orders with payment info)
  useEffect(() => {
    const loadPayments = () => {
      try {
        const savedOrders = JSON.parse(localStorage.getItem('bookings') || '[]');
        // Transform orders into payment records
        const paymentRecords = savedOrders.map(order => ({
          id: order.id,
          orderId: order.id,
          date: order.date,
          ...
          last4: order.paymentMethod === 'Credit Card' ? '4242' : null,
          billingAddress: order.shippingAddress || '123 Main St, City, State 12345',
  ...
  const processRefund = () => {
    ...
    try {
      ...
      localStorage.setItem('bookings', JSON.stringify(
        JSON.parse(localStorage.getItem('bookings') || '[]').map(order => 
          order.id === selectedPayment.id 
            ? { ...order, refundStatus: 'Processed', refundAmount: parseFloat(refundAmount) }
            : order
        )
      ));
      
      setSelectedPayment({ ...selectedPayment, refundStatus: 'Processed', refundAmount: parseFloat(refundAmount) });
      setShowRefundModal(false);
      setRefundAmount('');
      setRefundReason('');
      
      setNotification({
        message: `Refund of ₹${parseFloat(refundAmount).toFixed(2)} processed for payment #${selectedPayment.id}`,
        type: 'success'
      });
    } catch (error) {
