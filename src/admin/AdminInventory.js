...
import { FiPlus, FiEdit2, FiTrash2, FiSearch, FiFilter, FiSave, FiX, FiPackage, FiAlertTriangle, FiCheckCircle, FiTrendingUp, FiTrendingDown } from 'react-icons/fi';
...
const AdminInventory = () => {
      ...
      if (savedInventory) {
        ...
        const sampleInventory = [
          ...
          },
          {
            ...
            nextRestockDate: '2024-01-20',
            status: 'out-of-stock',
            notes: 'Out of stock, awaiting shipment'
          }
        ];
        setInventory(sampleInventory);
        localStorage.setItem('adminInventory', JSON.stringify(sampleInventory));
      }
    };
  ...
  const handleSubmit = (e) => {
    ...
    if (editingItem) {
      setInventory(prev => prev.map(item => 
        item.id === editingItem.id ? inventoryItem : item
      ));
    } else {
      setInventory(prev => [...prev, inventoryItem]);
    }
    
    localStorage.setItem('adminInventory', JSON.stringify(
      editingItem 
        ? inventory.map(item => item.id === editingItem.id ? inventoryItem : item)
  ...
  const getStockTrend = (item) => {
    ...
    if (quantity >= (item.maxStockLevel || minStockLevel * 2)) return <FiTrendingUp className="trend-icon up" />;
    return null;
  };
